    ...
    <DialogOverlay>
      <DialogPrimitive.Content
        ...
        className={cn(
          ...
          'grid w-full max-w-lg gap-4 border shadow-lg duration-200 sm:rounded-lg',
          'relative',
          'my-4',
          className
        )}
...
const DialogHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('flex flex-col space-y-1.5 px-4 py-3 text-center sm:text-left', className)} {...props} />
);
DialogHeader.displayName = 'DialogHeader';

const DialogFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('flex flex-col-reverse px-4 py-3 sm:flex-row sm:justify-end sm:space-x-2', className)} {...props} />
);
