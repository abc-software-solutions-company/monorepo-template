...
const FormLabel = React.forwardRef<React.ElementRef<typeof Text>, React.ComponentPropsWithoutRef<typeof Text>>((props, ref) => {
...
const FormMessage = React.forwardRef<React.ElementRef<typeof Text>, IFormMessageProps>(({ children, message, ...props }, ref) => {
  ...
  return (
    <Text ref={ref} key={formMessageId} fontSize={15} fontWeight="Medium" color={Colors.red[500]} style={ds.mt2} {...props}>
      {body}
    </Text>
  );
});
