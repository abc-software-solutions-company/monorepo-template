...
const Avatar: React.FC<AvatarProps> = ({ style, children, size = 40, onPress, ...props }) => {
  ...
  return (
    <AvatarContext.Provider value={{ isImageLoaded, setIsImageLoaded }}>
      <Pressable style={[ds.roundedFull, ds.overflowHidden, dynamicStyles.size(size), style]} onPress={onPress} {...props}>
        {children}
      </Pressable>
...
const AvatarFallback: React.FC<IAvatarFallbackProps> = ({ style, children, charsToShow = 1, ...props }) => {
  ...
  return (
    <View style={[ds.wFull, ds.hFull, ds.itemsCenter, ds.justifyCenter, dynamicStyles.background(configs.primary[500]), style]}>
