...
const Loading: FC<ILoadingProps> = ({ animationSpeed = 1000, fadeSpeed = 0, size = 24, thickness = 3, color, trackColor, visible = true }) => {
...
function startAnimation(rotateSpeed: number, fadeSpeed: number, fade: Animated.Value, rotation: Animated.Value) {
  ...
  Animated.loop(Animated.timing(rotation, { toValue: 360, duration: rotateSpeed, easing: Easing.linear, useNativeDriver: true })).start();
}
