...
DB_EXISTS=$(docker exec -e PGPASSWORD="$ADMIN_PASS" postgres psql -U "$ADMIN_USER" -d postgres -tAc "SELECT 1 FROM pg_database WHERE datname = '$DB_NAME'" || echo "")

if [ -z "$DB_EXISTS" ]; then
  echo "Creating database $DB_NAME..."
  docker exec -e PGPASSWORD="$ADMIN_PASS" postgres psql -U "$ADMIN_USER" -d postgres -c "CREATE DATABASE $DB_NAME;"
  
  echo "Creating user $DB_USER..."
  docker exec -e PGPASSWORD="$ADMIN_PASS" postgres psql -U "$ADMIN_USER" -d postgres -c "CREATE USER $DB_USER WITH PASSWORD '$DB_PASS';"
  
  echo "Granting privileges..."
  docker exec -e PGPASSWORD="$ADMIN_PASS" postgres psql -U "$ADMIN_USER" -d postgres -c "GRANT ALL PRIVILEGES ON DATABASE $DB_NAME TO $DB_USER;"
