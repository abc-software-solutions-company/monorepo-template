...
export class Application {
  ...
  async setupGlobals() {
    ...
    this.app.useStaticAssets(join(__dirname, '..', 'uploads'), { index: false });
  }
  ...
  async setupDocument() {
    ...
    if (isDocumentationEnabled) {
      const documentConfig = new DocumentBuilder()
        .setTitle('NestJs Admin Portal Template')
        .setDescription('The NestJs Admin Portal Template API description')
        .setVersion('1.0')
      ...
      const customOptions: SwaggerCustomOptions = {
        swaggerOptions: {
          defaultModelsExpandDepth: -1, // Hide section 'Schemas'
          /// More options are here: https://swagger.io/docs/open-source-tools/swagger-ui/usage/configuration/
        },
      };
      const document = SwaggerModule.createDocument(this.app, documentConfig, documentOptions);

      SwaggerModule.setup('documentation', this.app, document, customOptions);
    }
  }
