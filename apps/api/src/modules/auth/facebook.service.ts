...
export class FacebookService {
  ...
  async verifyAccessToken(token: string) {
    try {
      ...
      const response = await axios.get(`https://graph.facebook.com/debug_token?input_token=${appAccessToken}&access_token=${token}`);
      ...
      const profileResponse = await axios.get(`https://graph.facebook.com/me?fields=id,name,email,picture&access_token=${token}`);
