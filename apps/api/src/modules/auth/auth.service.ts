...
import { USER_GENDER, USER_ROLE, USER_STATUS } from '../users/constants/users.constant';
...
export class AuthService {
  ...
  async signUp(createUserDto: CreateUserDto) {
    const { email, password, name, gender, phoneNumber, avatar } = createUserDto;
    ...
    const createdUser = await this.usersService.create(null, {
      name,
      avatar,
      email,
      password,
    ...
    const emailResult = this.emailService.sendGmail(
      ...
      'Create Account',
      `Hello`,
      `Hello, your account has been created. Please login to continue.`
    );
  ...
  async resetPasswordForMobile(resetPasswordDto: ResetPasswordDto) {
    ...
    this.usersService.update(user.id, user, newUser);
  ...
  private async createNewUserFromOAuthProfile(provider: AUTH_PROVIDER, profile: OAuthProfile) {
