...
import slices from './posts.slice';

import PostApi from '../api/posts.api';

export function* list(action: PayloadAction<{ filter: PostFilter }>) {
  try {
    const response: AxiosResponse<PostsResponse> = yield call(() => PostApi.list(action.payload.filter));
...
export function* create(action: PayloadAction<PostFormData>) {
  try {
    const response: AxiosResponse<PostResponse> = yield call(() => PostApi.create(action.payload));
...
export function* read(action: PayloadAction<EntityId>) {
  try {
    const response: AxiosResponse<PostResponse> = yield call(() => PostApi.read(action.payload));
...
export function* update(action: PayloadAction<{ id: EntityId; data: PostFormData }>) {
  try {
    const response: AxiosResponse<PostResponse> = yield call(() => PostApi.update(action.payload.id, action.payload.data));
...
export function* destroy(action: PayloadAction<EntityId>) {
  try {
    const response: AxiosResponse<PostResponse> = yield call(() => PostApi.destroy(action.payload));
...
export function* bulkDestroy(action: PayloadAction<{ ids: EntityId[] }>) {
  try {
    const response: AxiosResponse<PostsResponse> = yield call(() => PostApi.bulkDestroy(action.payload));
