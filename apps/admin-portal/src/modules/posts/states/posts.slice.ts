...
import { PostEntity, PostFilter, PostFormData, PostResponse, PostsResponse } from '../interfaces/posts.interface';

export const entityAdapter = createEntityAdapter<PostEntity, EntityId>({
  selectId: item => item.id,
});

export interface IInitialState extends ReduxBaseState {
  ids: EntityId[];
  entities: { [key: string]: PostEntity };
  selected: EntityId[];
  filter?: PostFilter;
  detail?: PostEntity;
}
...
const slice = createSlice({
  ...
  reducers: {
    ...
    LIST
    *****************************************************************/
    listRequest(state, _action: PayloadAction<{ filter?: PostFilter }>) {
      state.isFetching = true;
    },
    listSuccess: (state, action: PayloadAction<PostsResponse>) => {
    ...
    CREATE
    *****************************************************************/
    createRequest: (state, _action: PayloadAction<PostFormData>) => {
      state.isCreating = true;
      state.error = undefined;
      state.message = undefined;
      state.statusCode = undefined;
    },
    createSuccess: (state, action: PayloadAction<PostResponse>) => {
    ...
    *****************************************************************/
    ...
    readSuccess: (state, action: PayloadAction<PostResponse>) => {
    ...
    UPDATE
    *****************************************************************/
    updateRequest: (state, _action: PayloadAction<{ id: EntityId; data: PostFormData }>) => {
      state.isUpdating = true;
      state.error = undefined;
      state.message = undefined;
      state.statusCode = undefined;
    },
    updateSuccess: (state, action: PayloadAction<{ params: unknown; response: PostResponse }>) => {
    ...
    *****************************************************************/
    ...
    destroySuccess: (state, action: PayloadAction<PostResponse>) => {
    ...
    *****************************************************************/
    ...
    bulkDestroySuccess(state, action: PayloadAction<PostsResponse>) {
      const { data, message, statusCode } = action.payload;
