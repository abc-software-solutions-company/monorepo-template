...
import { CreateFileDto, FileEntity, FileFilter, FileResponse, FilesResponse, UpdateFileDto } from '../interfaces/files.interface';

export const entityAdapter = createEntityAdapter<FileEntity, EntityId>({
  selectId: item => item.id,
});

export interface IInitialState extends ReduxBaseState {
  ids: EntityId[];
  entities: { [key: string]: FileEntity };
  selected: EntityId[];
  filter?: FileFilter;
  detail?: FileEntity;
}
...
const slice = createSlice({
  ...
  reducers: {
    ...
    LIST
    *****************************************************************/
    listRequest(state, _action: PayloadAction<{ filter?: FileFilter }>) {
      state.isFetching = true;
    },
    listSuccess: (state, action: PayloadAction<FilesResponse>) => {
    ...
    CREATE
    *****************************************************************/
    createRequest: (state, _action: PayloadAction<CreateFileDto>) => {
      state.isCreating = true;
      state.error = undefined;
      state.message = undefined;
      state.statusCode = undefined;
    },
    createSuccess: (state, action: PayloadAction<FileResponse>) => {
    ...
    *****************************************************************/
    ...
    readSuccess: (state, action: PayloadAction<FileResponse>) => {
    ...
    UPDATE
    *****************************************************************/
    updateRequest: (state, _action: PayloadAction<{ id: EntityId; data: UpdateFileDto }>) => {
      state.isUpdating = true;
      state.error = undefined;
      state.message = undefined;
      state.statusCode = undefined;
    },
    updateSuccess: (state, action: PayloadAction<{ params: unknown; response: FileResponse }>) => {
    ...
    *****************************************************************/
    ...
    destroySuccess: (state, action: PayloadAction<FileResponse>) => {
    ...
    *****************************************************************/
    ...
    FILTER
    *****************************************************************/
    setFilter(state, action: PayloadAction<FileFilter>) {
