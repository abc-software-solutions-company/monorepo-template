...
const pricingList: IPricingProps[] = [
  {
    title: 'Free',
    popular: 0,
    price: 0,
...
const Pricing: FC<PricingProps> = ({ className }) => {
  return (
    <section id="pricing" className={classNames('py-14 sm:py-16', className)}>
      <div className="container">
        <h2 className="text-center text-3xl font-bold md:text-4xl">
          Get
          <span className="bg-gradient-to-b from-primary/60 to-primary bg-clip-text text-transparent"> Unlimited </span>
          Access
        </h2>
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {pricingList.map((pricing: IPricingProps) => (
            <Card
              key={pricing.title}
              className={pricing.popular === PopularPlanType.YES ? 'shadow-black/10 drop-shadow-xl dark:shadow-white/10' : ''}
            >
              <CardHeader>
                ...
                <div className="space-y-4">
                  {pricing.benefitList.map((benefit: string) => (
                    <span key={benefit} className="flex">
                      <CheckIcon className="text-green-500" /> <h3 className="ml-2">{benefit}</h3>
                    </span>
                  ))}
                </div>
              </CardFooter>
